...
class VAE:
    ...
    def encoder(self, x):
        ...
        log_alpha = T.dot(h_encoder, self.params['W_halpha']) + self.params['b_halpha'].dimshuffle('x', 0)
        log_beta = T.dot(h_encoder, self.params['W_hbeta']) + self.params['b_hbeta'].dimshuffle('x', 0)

        alpha =  T.exp(log_alpha)+ epsilon
        beta = T.exp(log_beta) + epsilon
