...
class VAE:
    ...
    def sampler(self, alpha, beta):
        ...
        # Reparametrize
        mu = alpha/(alpha + beta)
        log_sigma = T.log(alpha) + T.log(beta) - 2*T.log(alpha + beta) - T.log(1+alpha+beta)

        z = mu + T.exp(0.5 * log_sigma) * eps

        #z = srng.binomial(size=alpha.shape, n=1, p=alpha)

        #z = np.random.beta(alpha, beta, size=alpha.shape)
    ...
    def create_gradientfunctions(self, x_train):
        ...
        KLD = 0.5 * T.sum((alpha/(alpha+beta)-0.5)**2, axis=1, keepdims=True)
        #KLD = 0 #temporary
